<?php 
	...
	//sql指令字串
	$sql = "SELECT id,header,writeTime FROM message_box WHERE area = 6";
	$sql1 = "SELECT id,post,postTime FROM webtobox WHERE areafk = 6";
				...
				<div class="container-fluid">
					<nav class="nav">						
						<ul class="nav nav-tabs">
								...
								<li class="active"><a href="./wetalk6.php">Mosasaurus</a></li>
							</ul>						
					</nav>
				</div>
			</div>
					...
					<table class="table table-striped table-hover table-responsive">
						...
						<tbody>
								  <?php for ($i=0; $i < count($test1) ; $i++) { ?>
									<?php echo "<tr>"; ?>
									<?php for ($j=0; $j < 1 ; $j++) { ?>
										 <?php echo '<th><a href="./wetalkcontent6.php?&id='.$test1[$i][0].'" id="'.$i.'">'.$test1[$i][1].'</a></th>'; ?>  
